        ...
        _data.read('users', phone, (err, data) => {
            if (err) {
                ...
                if (hashPassword) {
                    ...
                    callback(500, {
                        'Error': 'Error creating the hashedPassword'
                    });
                }
            } else {
                callback(400, {
                    'Error': 'A User with that phone number exists'
                });
            }
        })
    } else {
    ...
    if (phone) {
        ...
        handlers._tokens.verifyToken(tokenId, phone, (tokenIsValid) => {
            if (tokenIsValid) {
                ...
                callback(403, {
                    'Error': 'Missing requried token in header / token invalid'
                });
            }
        });
    } else {
        ...
        if (firstName || lastName || password) {
            ...
            _data.read('users', phone, (error, userData) => {
                if (!error && userData) {
                    ...
                    callback(400, {
                        'Error': 'The specified phone does not exist'
                    });
                }
            });
        } else {
            ...
            if (!err && data) {
                ...
                _data.delete('users', phone, (err) => {
                    if (!err) {
                        ...
                        if (checksToDelete > 0) {
                            const checkDeleted = 0;
                            const deletionError = false;
                            userChecks.forEach((id,element)=>{
                                // Delete the check
                                ...
                                if(checkDeleted == checksToDelete) {
                                    if(!deletionError) {
                                        ...
                                        callback(500, {'Error': 'Errors encountered while deleting the checks'});
                                    }
                                }
                            });
                        } else {
                ...
                callback(400, {
                    'Error': 'Couldnot find the specified phone'
                });
            }
        })
    } else {
                ...
                if (userData.hashPassword == hashPass) {
                    ...
                    _data.create('tokens', tokenId, tokenObj, (err) => {
                        if (!err) {
                            ...
                            callback(500, {
                                'Error': 'Error storing the token object'
                            });
                        }
                    });
    ...
    if (tokenId && extend) {
        ...
        _data.read('tokens', tokenId, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                callback(400, {
                    'Error': 'Token ID doesn\'t exists'
                });
            }
        })
    } else {
        callback(400, {
            'Error': 'Missing required field(s) / Invalid field(s)'
        });
    }
};
    ...
    if (id) {
        _data.read('tokens', id, (err, data) => {
            if (!err && data) {
                ...
                callback(400, {
                    'Error': 'Couldnot find the specified token id'
                });
            }
        });
    } else {
...
handlers._checks.post = (data, callback) => {
    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
        ...
        callback(400, {
            'Error': 'Missing requried inputs / Inputs are invalid'
        });
    }
}
...
handlers._checks.get = (data, callback) => {
    ...
    const id = typeof (data.queryStringObject.id) == 'string' && data.queryStringObject.id.trim().length == 20 ? data.queryStringObject.id.trim() : false;
    if (id) {
        //Lookup up the checks 
        _data.read('checks', id, (err, checkData) => {
            if (!err && checkData) {
                //Verify that the given token from the headers is valid and belongs to the user who created it
...
// CHECKS - PUT
// Required Parameters - id
// Optional Parameters - protocol, url, method, success, timeouts - 
// Atleast one optional paramaters must be set
handlers._checks.put = (data, callback) =>{
    ...
    if(id) {
                ...
                if(!error && checkData) {
                    ...
                    handlers._tokens.verifyToken(token, checkData.userPhone, (tokenIsValid) => {
                        if (tokenIsValid) {
                            ...
                            protocol ? checkData.protocol = protocol : checkData.protocol = '';
                            method ? checkData.method = method : checkData.method = '';
                            url ? checkData.url = url : checkData.url = '';
                            successCodes ? checkData.successCodes = successCodes : checkData.successCodes = '';
                            timeoutSeconds ? checkData.timeoutSeconds = timeoutSeconds : checkData.timeoutSeconds = '';
                            // Store the updates
                            _data.update('checks', id, checkData, (err)=>{
                                if(!err) {
                                    callback(200, {'Success':'Checks updated'});
                                } else {
                                    callback(500, {'Error':'Could not update the check'});
                                }
                            });
                        } else {
                            callback(403, {'Error':'Missing header token'});
                        }
                    });
                } else {
        ...
        callback(400, {'Error':'Missing required field - Check ID'});
    }
...
handlers._checks.delete = (data, callback) =>{
    ...
    const id = typeof (data.payload.id) == 'string' && data.payload.id.trim().length == 20 ? data.payload.id.trim() : false;
    if (id) {
        // Lookup the check that you want to delete
        _data.read('checks', id, (err,checkData)=>{
            if (!err && checkData) {
                const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
                handlers._tokens.verifyToken(token, checkData.userPhone, (tokenIsValid) => {
                    if( tokenIsValid ) {
                        // Delete the check data from collection
                        _data.delete('checks',id,(err)=>{
                            if(!err) {
                                // Delete the user
                                _data.read('users',checkData.userPhone, (err,userData)=>{
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            _data.update('users',checkData.userPhone, (err,userData)=>{
                                                if (!err && userData) {
                                                    callback(200);
                                                } else {
                        ...
                        callback(400, {'Error':'Token verification failed'});
                    }
                });
            } else {
